...
type GenderScopeParam = GenderScope | "all" | "men-women";
...
function isShoeLikeProduct(p: Product) {
  const name = String((p as any).name || "").toLowerCase();
  const slug = String((p as any).slug || "").toLowerCase();
  const category = String((p as any).category || "").toLowerCase();
  const hay = [name, slug, category].join(" ");

  const blocked = [
    "cap", "hat", "gorra", "beanie", "visor", "bucket",
    "bag", "backpack", "mochila", "wallet", "belt", "cinturon", "calcetin", "sock", "socks",
    "legging", "leggings", "pants", "pant", "short", "shorts", "top", "tank", "bra",
    "hoodie", "jacket", "tee", "t-shirt", "shirt", "camiseta", "sudadera", "fleece", "jogger",
    "sweatpant", "sports bra", "training top", "club cap", "accessor", "accesorio", "clothing", "ropa"
  ];
  if (blocked.some((k) => hay.includes(k))) return false;

  const shoeHints = [
    "shoe", "shoes", "sneaker", "sneakers", "zapatilla", "zapatillas",
    "dunk", "air force", "air max", "jordan", "retro", "vomero", "pegasus", "v2k",
    "samba", "gazelle", "campus", "forum", "superstar", "yeezy", "slide", "slipper", "running"
  ];
  if (shoeHints.some((k) => hay.includes(k))) return true;

  return category.includes("shoe") || category.includes("sneaker") || category.includes("zapat");
}
